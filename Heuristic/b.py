...
class Board():
	...
	def __init__(self, algo, placePieces=None):
		self.algo = algo
		if placePieces:
			print()
			self.placedPieces = placePieces
		else:
	...
	def putPiece(self, caseId, piece, rotation, correct):
		(x,y) = self.algo.getCoordinates(caseId)
		self.placedPieces[x][y] = (piece, rotation, correct)
