...
class Algo:
    ...
    def runStack(self):
        ...
        while not self.stop:
            ...
            self.currentBoard.putPiece(case, piece, rotation, 0)    
            self.remainingPieces.remove(piece)
            ...
            if case == 255:
                ...
                if (self.gui.time == "0:00:15"):
                    # 15 seconds have passed since last best solution found
                    print("time is up")
                    if (len(listMoves) == 0):
                        self.continueFromRecord()
    ...
    def continueFromRecord(self):
        for caseID in range(len(self.currentBoard.getListPieces())):
            self.currentBoard.removePiece(caseID)
        
        self.remainingPieces.clear()
        self.remainingPieces = self.listPieces.copy()
            
            
        caseID = 0    
        for row in self.recordBoard.placedPieces:
            try:
                for(piece, rotation, correct) in row:
                    self.currentBoard.putPiece(caseID, piece, rotation, correct)
                    caseID += 1
            except TypeError:
                pass
            self.remainingPieces.remove(piece)
    ...
    def cost(self, board):
        # return the number of errors in the board
        errors = 0
        for row in board:
            try:
